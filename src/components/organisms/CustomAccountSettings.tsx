  ...
  return (
    <div className="flex max-w-3xl flex-col gap-8">
      ...
      <div className="space-y-4">
          ...
          {user.emailAddresses.map((email) => (
            <div key={email.id} className="flex items-center gap-2">
              ...
              {user.primaryEmailAddressId === email.id && (
                <Badge
                  variant="secondary"
                  className="bg-red-100 text-xs font-normal text-red-600 hover:bg-red-100"
                >
                  Primary
                </Badge>
              )}
        ...
        <Button
          variant="ghost"
          className="h-auto justify-start p-0 font-normal text-red-500 hover:bg-red-50 hover:text-red-600"
        >
          + Add Email Address
        </Button>
      </div>
          ...
          <div className="space-y-6">
            {sessions?.map((session) => {
              const sessionWithActivity =
                session as unknown as SessionWithActivity;
              return (
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-medium">
                        {sessionWithActivity.latestActivity?.deviceType ??
                          "Device"}
                      </p>
                      {session.status === "active" && (
                        <Badge
                          variant="secondary"
                          className="h-5 rounded-sm bg-red-100 px-1.5 py-0 text-xs font-normal text-red-600 hover:bg-red-100"
                        >
                          Current Device
                        </Badge>
                      )}
                    </div>
                    {(sessionWithActivity.latestActivity?.browserName ??
                      sessionWithActivity.latestActivity?.browserVersion) && (
                      <p className="text-muted-foreground text-xs">
                        {sessionWithActivity.latestActivity?.browserName}{" "}
                        {sessionWithActivity.latestActivity?.browserVersion}
                      </p>
                    )}
                    {(sessionWithActivity.latestActivity?.city ??
                      sessionWithActivity.latestActivity?.country) && (
                      <p className="text-muted-foreground text-xs">
                        {sessionWithActivity.latestActivity?.city},{" "}
                        {sessionWithActivity.latestActivity?.country}
                      </p>
